/* Service worker v7.5.5 : réseau d'abord, cache hors-ligne.
   Charge dashboard, réparation des mois, auth fluide, dashboard premium cash
   et synchronisation automatique des GID Google pour les liens de mois du Sheet.
   Recharge les fenêtres ouvertes à l'activation pour éviter un ancien cache. */
const CACHE = 'budget-saisie-v7-5-5';
const SHELL = ['./', './index.html', './app.js', './patch-v75.js', './patch-v751.js', './patch-v753.js', './patch-v754.js', './patch-v755.js', './manifest.webmanifest', './icon-192.png', './icon-512.png', './icon-180.png'];
...
async function patchedApp(req){
  ...
  const [p75,p752,p753,p754,p755]=await Promise.all([
