...
/* Nouveau modèle d'état : plusieurs années peuvent être actives simultanément. */
dash={years:[new Date().getFullYear()],month:null,status:'realized',topAll:false};

/* ---------- Création d'onglets mensuels dans l'ordre physique ---------- */
...
/* ---------- Graphique adaptatif : 12, 24, 36 mois... ---------- */
...
renderDashboard=async function(){
  ...
  if(!dash.years.length){
    const cy=new Date().getFullYear();
    dash.years=[years.includes(cy)?cy:years[years.length-1]];
  }
  ...
  years.forEach(yv=>{
    ...
    b.onclick=()=>{
      if(on){ if(dash.years.length===1) return; dash.years=dash.years.filter(y=>y!==yv); }
      else dash.years=[...dash.years,yv].sort((a,b)=>a-b);
      renderDashboard();
    };
    yc.appendChild(b);
  });
  ...
  allBtn.onclick=()=>{
    dash.years=dash.years.length===years.length?[years[years.length-1]]:years.slice();
    dash.month=null;
    renderDashboard();
  };
  ...
  const allM=el('button','chip'+(dash.month==null?' on':''),'Tous les mois');
  allM.onclick=()=>{dash.month=null;renderDashboard();}; mc.appendChild(allM);
  availableMonths.forEach(m=>{
    const b=el('button','chip'+(dash.month===m?' on':''),MONTHS_SHORT[m]);
    b.onclick=()=>{dash.month=m;renderDashboard();}; mc.appendChild(b);
  });

  if(sc){
    sc.innerHTML='';
    [['realized','Réalisé'],['forecast','Prévisionnel'],['all','Tout']].forEach(([v,l])=>{
      const b=el('button','chip'+(dash.status===v?' on':''),l);
      b.onclick=()=>{dash.status=v;renderDashboard();}; sc.appendChild(b);
    });
  }

  const selectedTabs=v75SelectedTabs();
  const previousYearTabs=(dash.years.length===1)
    ? allTabs.filter(t=>t.y===dash.years[0]-1&&v75StatusOk(t)&& (dash.month==null||t.m===dash.month))
    : [];
  ...
  if(dash.month!=null&&dash.years.length===1){
    open.style.display='block'; open.textContent=`Ouvrir ${MONTHS_FR[dash.month]} ${dash.years[0]} dans Mois ›`;
    open.onclick=()=>{moisDate=new Date(dash.years[0],dash.month,1);switchTab('mois');};
  } else open.style.display='none';

  const cc=$('#dash-compare-card');
  if(dash.years.length>1){
    cc.style.display='block';
    $('#dash-compare-title').textContent='📊 Synthèse par année';
    $('#dash-compare-note').style.display='none';
    let html='';
    dash.years.forEach(y=>{
      const a=v75YearAggregate(list,y), r=a.ent?a.sol/a.ent:0;
      html+=`<div class="year-summary"><div class="ys-year">${y}</div><div>Entrées<br><b>${fmtEUR0(a.ent)}</b></div><div>Sorties<br><b>${fmtEUR0(a.sor)}</b></div><div class="ys-solde ${a.sol>=0?'pos':'neg'}">Solde ${fmtEUR0(a.sol)} · ${(r*100).toFixed(1).replace('.',',')} %</div></div>`;
    });
    $('#dash-compare').innerHTML=html;
  } else if(previousYearTabs.length){
    const prev=aggStats(previousYearTabs.map(t=>({...t,stat:stats[t.title]})));
    cc.style.display='block';
    $('#dash-compare-title').textContent=`🔁 ${dash.years[0]} vs ${dash.years[0]-1}`;
    $('#dash-compare-note').style.display='none';
    $('#dash-compare').innerHTML=cmpRow('Dépenses',scope.sor,prev.sor,false)+cmpRow('Revenus',scope.ent,prev.ent,true)+cmpRow('Épargne',scope.sol,prev.sol,true);
  } else cc.style.display='none';

  let chartItems=list.map(x=>({
    label:MONTHS_AB[x.m]+String(x.y).slice(2),
    title:`${MONTHS_FR[x.m]} ${x.y}`,
    ent:x.stat.ent,sor:x.stat.sor,sol:x.stat.sol,y:x.y,m:x.m
  }));
  if(dash.month!=null){
    chartItems=dash.years.map(y=>{
      const x=list.find(z=>z.y===y&&z.m===dash.month);
      return x?{label:String(y),title:`${MONTHS_FR[dash.month]} ${y}`,ent:x.stat.ent,sor:x.stat.sor,sol:x.stat.sol,y,m:dash.month}
              :{label:String(y),title:`${MONTHS_FR[dash.month]} ${y}`,ent:0,sor:0,sol:0,y,m:dash.month};
    });
  }
  ...
  ch.querySelectorAll('[data-tap]').forEach(r=>r.onclick=()=>{
    const it=chartItems[+r.dataset.tap]; if(!it) return;
    moisDate=new Date(it.y,it.m,1); switchTab('mois');
  });
  ...
  shown.forEach((c,i)=>{
    const row=el('div','rank'); row.appendChild(el('div','rn',String(i+1)));
    const mid=el('div','rmid'); mid.appendChild(el('div','rl',c.l));
    const bar=el('div','rbar'),fill=el('i'); fill.style.width=Math.max(2,Math.round(c.amt/maxC*100))+'%'; bar.appendChild(fill); mid.appendChild(bar); row.appendChild(mid);
    const rr=el('div','rr'); rr.appendChild(el('div','ra',fmtEUR0(c.amt))); rr.appendChild(el('div','rp',Math.round(c.amt/totS*100)+' %')); row.appendChild(rr);
    row.onclick=()=>openCatYear(c.l); top.appendChild(row);
  });
  ...
  secVals.forEach(x=>{
    const row=el('div','brk'); row.appendChild(el('div','bl',x.name));
    const bar=el('div','bar'),i2=el('i'); i2.style.width=Math.round(x.val/maxSec*100)+'%'; i2.style.background=x.color; bar.appendChild(i2); row.appendChild(bar);
    row.appendChild(el('div','bv',Math.round(x.val/totSec*100)+'% · '+fmtEUR0(x.val))); brk.appendChild(row);
  });
};

/* Détail catégorie compatible avec plusieurs années / prévisionnel. */
openCatYear=function(label){
  ...
  let tot=0,nb=0;
  tabs.forEach(t=>{
    const c=monthCache[t.title]; if(!c||!c.stat) return;
